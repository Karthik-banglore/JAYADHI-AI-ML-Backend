...
method: 'POST',
headers: { 'Content-Type': 'application/json' },
body: JSON.stringify({ image\_ imageData, guess: guess })
});
...
1. **chatbot upgradation mentioned above**

2. **Prompt Evaluator Game** – New educational game for prompt writing practice.
...
Let students practice writing prompts for creative AI tasks. The backend (powered by an LLM) evaluates their prompt for clarity, creativity, and fit, and gives a score and feedback.

### **Request**

{“level”: “easy”, // “easy”, “moderate”, or “hard”
“task_index”: 0, // integer: which prompt to use from the list
“user_prompt”: “Draw a cute cat flying a big red kite in the sky.” // student’s prompt
}
...
Students give a sequence of three numbers. The backend (powered by an LLM) predicts the next number and explains the logic in a kid-friendly way.
...
- CORS is enabled; you can call these APIs directly from your frontend.
- For the prompt evaluator, the backend will always return the actual task assigned, the score (`result`), and detailed feedback.
- For the pattern predictor, the backend always returns the predicted next number and a clear, step-by-step explanation.
- If you get an error, check for missing or invalid fields in your request.
...
async function evaluatePrompt(level, taskIndex, userPrompt) {
  const res = await fetch(’/api/game/prompt-evaluator’, {
    method: ‘POST’,
    headers: {‘Content-Type’: ‘application/json’},
    body: JSON.stringify({level, task_index: taskIndex, user_prompt: userPrompt})
  });
  return res.json();
}
```
...
**Prompt Evaluator Response:**
{“task”: “You wake up and can talk to animals. What do you say to your pet?”,“result”: “25%”,“feedback”: “This prompt doesn’t follow the instruction of creating a story about waking up and talking to animals. While it’s creative and clear on its own, it completely misses the point of the original assignment. To fix this, the prompt needs to include waking up, talking to animals, and a pet. For example: "I wake up and my dog is staring at me. I can suddenly understand him! What do I say?".”}


**Pattern Predictor Response:**
{“next_number”: 40,“reasoning”: “The sequence increases by 10 with each subsequent number (10, 20, 30…). Therefore, the next number is 30 + 10 = 40.”}


**If you have any questions, check this guide first. If you still need help, contact the backend team.**

This guide is ready for your frontend team—**no ambiguity, no missing info, and includes real tested results**.
This ensures a smooth integration experience with our AI backend services.